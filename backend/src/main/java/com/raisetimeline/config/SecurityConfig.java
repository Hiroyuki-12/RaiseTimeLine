 ...
 *
 * Spring Security は Web アプリのセキュリティ機能（認証・認可・CORS・CSRF 等）を提供するフレームワーク。
 * ここで「どのエンドポイントに認証が必要か」「どの認証方式を使うか」などを設定する。
 *
 * 注意: Spring Boot 4.x（Spring Security 7.x）では WebSecurityConfigurerAdapter クラスが削除された。
 * 代わりに SecurityFilterChain を @Bean として定義する方式が標準になっている。
 */
@Configuration
...
public class SecurityConfig {
    ...
    /**
     * セキュリティのルールを定義するメインメソッド。
     * HTTP リクエストに対するセキュリティポリシーを設定する。
     *
     ...
     *
     * BCryptPasswordEncoder を使う。BCrypt の特徴:
     *   - 一方向ハッシュ（元のパスワードに戻せない）
     *   - ソルト（ランダムデータ）を自動生成してハッシュに組み込む
     *     → 同じパスワードでもハッシュ値が毎回異なる（レインボーテーブル攻撃を防ぐ）
     *   - コスト係数（デフォルト: 10）でハッシュ計算の遅さを調整できる
     *     → 遅いほどブルートフォース攻撃に強い
     *
     * @Bean として登録することで AuthService に DI（依存性注入）できる。
     */
    @Bean

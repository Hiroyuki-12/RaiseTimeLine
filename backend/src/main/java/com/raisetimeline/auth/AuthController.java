 ...
 *
 * <p>SecurityConfig で /api/auth/** は認証不要（permitAll）に設定しているため、
 * 未ログインのユーザーもこれらのエンドポイントにアクセスできる。
 *
 * <p>リフレッシュトークンのセキュリティ設計:
 * - HttpOnly Cookie に保存 → JavaScript からアクセス不可（XSS 対策）
 * - SameSite=Lax → 他サイトからの POST リクエストでは Cookie が送られない（CSRF 対策）
 * - Path=/api/auth → リフレッシュ・ログアウト以外のリクエストでは Cookie が送られない（スコープを最小化）
 */
@RestController
...
public class AuthController {
     ...
     * @return 201 Created + AuthResponse（アクセストークン・ユーザーID・ユーザー名）
     *         リフレッシュトークンは Set-Cookie ヘッダーで HttpOnly Cookie として送る
     */
     ...
     * @return 200 OK + AuthResponse（アクセストークン・ユーザーID・ユーザー名）
     *         リフレッシュトークンは Set-Cookie ヘッダーで HttpOnly Cookie として送る
     */
    @PostMapping("/login")
     ...
     *
     * <p>フロントエンドのアクセストークン（15分）が期限切れになったとき、このエンドポイントを呼ぶ。
     * Cookie に含まれるリフレッシュトークンを DB で検証し、有効なら新しいアクセストークンを返す。
     * リフレッシュトークンもローテーション（更新）される。
     *
     ...
     *
     * <p>DB からリフレッシュトークンを削除し、Cookie を無効化（Max-Age=0）する。
     * アクセストークンはクライアント側のメモリから削除する（サーバーでは管理しない）。
     *
     * @param refreshToken Cookie "refresh_token" の値（@CookieValue で自動取得）
    ...
    /**
     * リフレッシュトークン用の HttpOnly Cookie を生成する内部メソッド。
     * HttpOnly: JavaScript からアクセス不可（XSS 対策）
     * SameSite=Lax: 別サイトの POST リクエストでは送られない（CSRF 対策）
     * Path=/api/auth: このパス配下のリクエストにのみ Cookie が付く（スコープ最小化）
     *

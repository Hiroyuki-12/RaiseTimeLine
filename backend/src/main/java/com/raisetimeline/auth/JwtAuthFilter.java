 ...
 * JWT 認証フィルター。
 *
 * HTTP リクエストが届くたびにこのフィルターが実行され、Authorization ヘッダーの JWT を検証する。
 * Spring Security のフィルターチェーンに組み込まれており、コントローラーに到達する前に実行される。
 *
 * OncePerRequestFilter を継承することで、1リクエストにつき1回だけ実行されることが保証される。
 *
 * 処理フロー:
 *   1. リクエストヘッダーから "Authorization: Bearer <token>" を取得
 *   2. JWT を検証（署名が正しい かつ 有効期限内か）
 *   3. 有効なら SecurityContextHolder に認証情報をセット（認証済み状態にする）
 *   4. 無効・なしの場合は何もしない（SecurityConfig の authorizeHttpRequests が 401 を返す）
 */
@Component

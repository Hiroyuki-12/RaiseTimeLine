 ...
 *
 * Java の Record クラスを使用している。Record は不変（immutable）なデータ入れ物で、
 * フィールド宣言と同時にコンストラクタ・getter・equals・hashCode・toString が自動生成される。
 *
 * jakarta.validation アノテーションによりバリデーション（入力値の検証）を宣言的に定義する。
 * コントローラーで @Valid を付けると自動でチェックが実行され、
 * 違反があれば GlobalExceptionHandler が 400 Bad Request を返す。
 */
public record RegisterRequest(

        /**
         * ユーザー名: 1〜50文字、英数字・アンダースコア・ハイフンのみ使用可。
         * 画面に表示される名前で、重複不可。
         */
        ...
        /**
         * パスワード: 8文字以上、英字と数字を両方含むこと。
         * 正規表現の意味:
         *   (?=.*[a-zA-Z]) : 英字を1文字以上含む（先読みアサーション）
         *   (?=.*\d)       : 数字を1文字以上含む（先読みアサーション）
         *   .+             : 任意の文字が1文字以上続く
         */
        ...
        /**
         * パスワード確認用: フロントで入力したパスワードと一致することを AuthService で確認する。
         * バリデーションアノテーションでは一致チェックができないため、
         * サービス層で password と比較する。
         */

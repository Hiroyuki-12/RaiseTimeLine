...
/**
 * ユーザー登録・ログインのビジネスロジックを担当するサービスクラス。 @Service アノテーションにより Spring が管理する Bean として登録される。
 * コントローラーからこのクラスを呼び出すことで、 HTTP の詳細（リクエスト・レスポンス）とビジネスロジックを分離できる。
 */
@Service
public class AuthService {
    ...
    /** コンストラクタ。Spring が依存オブジェクトを自動的に注入する（依存性注入）。 @Autowired なしでもコンストラクタが1つの場合は自動でDIが機能する。 */
     ...
     */
    /*
     * @Transactional: このメソッド内の DB 操作（INSERT）をトランザクションで囲む。
     * 途中で例外が発生した場合、DB への変更が自動的にロールバックされる。
     * 書き込み操作には必ず付けることで、データの整合性を保証する。
     */
    @Transactional
     ...
     * ログインを行い、JWT トークンを返す。
     *
     * <p>処理の流れ: 1. メールアドレスでユーザーを検索（存在しなければ 401） 2. 入力パスワードと DB のハッシュを BCrypt で照合（不一致なら 401） 3. JWT
     * を生成してレスポンスに含める
     *
